...
LocalLockManager::LocalLockManager(int node_id, int num_nodes, int num_objects,
  ...
  memset((void*)current_exclusive_owner_, 0x00, sizeof(volatile int)*num_nodes*num_objects_);
...
int LocalLockManager::TryLock(int target_node_id, int target_obj_index, int owner_user_id,
  ...
  if (lock_status_[idx] != LOCK_STATUS_IDLE && lock_status_[idx] != LOCK_STATUS_FULL) {
    ret = LOCAL_LOCK_FAIL;
  } else {
    if (lock_type == SHARED) {
      if (current_exclusive_owner_[idx] > 0) {
        ...
        ret = LOCAL_LOCK_FAIL;
      } else if (shared_counter_[idx] == 0 && current_exclusive_owner_[idx] == 0) {
      ...
      if (current_exclusive_owner_[idx] == owner_user_id && shared_counter_[idx] == 0) {
        ret = LOCAL_LOCK_PASS;
      } else if (current_exclusive_owner_[idx] != 0 && shared_counter_[idx] == 0) {
        // Ex -> Ex
        ...
        next_exclusive_owner_[idx][(int)last_exclusive_owner_[idx]] = owner_user_id;
        ...
        if (shared_counter_[idx] > 0)
          ++shared_to_exclusive_fail_count_;
        ret = LOCAL_LOCK_FAIL;
      } else {
        lock_status_[idx] = LOCK_STATUS_LOCKING;
        ret = LOCAL_LOCK_RETRY;
  ...
  if (lock_status_[idx] != LOCK_STATUS_IDLE && lock_status_[idx] != LOCK_STATUS_FULL) {
    ...
    if (lock_type == SHARED) {
      ...
      if (last_exclusive_owner_[idx] == owner_user_id) {
        ...
        current_exclusive_owner_[idx] = next_exclusive_owner_[idx][owner_user_id];
        next_exclusive_owner_[idx][owner_user_id]                      = 0;
...
int LocalLockManager::LeaveExclusive(int target_node_id, int target_obj_index, int owner_user_id) {
  int ret = 0;
  int idx = index(target_node_id, target_obj_index);
  mutex_[idx].lock();
