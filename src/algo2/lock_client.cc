    ...
    if (context->last_lock_task == LockManager::TASK_LOCK) {
      if (context->last_lock_type == LockManager::EXCLUSIVE) {
        if (exclusive == 0 && shared == 0) {
          ...
          context->last_read_target = LockManager::SHARED;
          ...
          context->last_read_target = LockManager::EXCLUSIVE;
          if (exclusive & waitlist_[context->last_obj_index] == 0) {
            waitlist_[context->last_obj_index] = 0;
          }
          if (waitlist_[context->last_obj_index] == 0) {
            context->waiters = exclusive;
            context->last_read_target = LockManager::EXCLUSIVE;
            //context->ignore = true;
            this->ReadRemotely(context,
                context->last_user_id,
...
//#endif
    if (context->last_read_target == LockManager::SHARED) {
      ...
      // Polling on Ex_X -> proceed if Ex_X & waiters is zero
