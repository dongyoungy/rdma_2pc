...
// 8 byte buffer space that each test client uses.
struct TestBuffer {
  uint64_t data;
};
...
struct Context {
  ...
  uint64_t* server_buffer;
  struct ibv_mr* rdma_server_buffer;
  uint64_t* client_buffer;
  struct ibv_mr* rdma_client_buffer;
