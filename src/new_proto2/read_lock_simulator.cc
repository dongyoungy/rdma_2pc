#include "read_lock_simulator.h"

namespace rdma { namespace proto {

ReadLockSimulator::ReadLockSimulator(LockManager* manager, uint32_t id, uint32_t home_id,
...
void ReadLockSimulator::StartLockRequests() {
...
void ReadLockSimulator::CreateLockRequests() {
...
void ReadLockSimulator::SubmitLockRequest() {
  ...
  if (current_request_idx_ < request_size_) {
    if (verbose_) {
      ...
      cout << "(REMOTE) Read Lock Simulator " << id_ << ": " << "Sending lock request #" <<
...
void ReadLockSimulator::SubmitUnlockRequest() {
