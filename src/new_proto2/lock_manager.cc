...
int LockManager::SendLockRequestResult(Context* context, int seq_no, uint32_t owner_user_id,
...
int LockManager::Lock(int seq_no, uint32_t user_id, uint32_t manager_id, int lock_type,
  ...
  // this was experimental for adaptive algorithm...
...
int LockManager::Unlock(int seq_no, uint32_t user_id, uint32_t manager_id, int lock_type,
    int obj_index) {
  int ret = llm_->TryUnlock(manager_id, obj_index, user_id, lock_type);
  ...
  if (ret == LOCAL_LOCK_PASS) {
    ...
    if (lock_mode_ == LOCK_PROXY_QUEUE || lock_mode_ == LOCK_REMOTE_NOTIFY) {
      LockClient* lock_client = lock_clients_[manager_id];
      ret = lock_client->RequestUnlock(seq_no, user_id, lock_type, obj_index,
          lock_mode_table_[manager_id]);
    }
...
int LockManager::LockLocallyWithRetry(Context* context, Message* message) {
  ...
  if (context) {
    ...
    if (SendLockRequestResult(context, seq_no, owner_user_id, lock_type, obj_index,
          lock_result)) {
...
int LockManager::LockLocallyWithQueue(Context* context, Message* message) {
  ...
  if (queue->GetSize() > 0) {
    ...
    queue->Insert(seq_no, target_node_id, owner_node_id, owner_user_id, lock_type);
...
int LockManager::UnlockLocallyWithRetry(Context* context, Message* message) {
  ...
  // send result back to the client
  if (context) {
    if (SendUnlockRequestResult(context, seq_no, owner_user_id, lock_type, obj_index,
          lock_result)) {
...
int LockManager::UnlockLocallyWithQueue(Context* context, Message* message) {
  ...
  if (lock_result == RESULT_SUCCESS) {
    ...
    if (elem) {
      ...
      lock_value                  = (uint32_t)pow(2.0, elem->owner_node_id);
      exclusive                   = (uint32_t)((*lock_object)>>32);
      shared                      = (uint32_t)(*lock_object);
      if (exclusive == 0 && elem->type == SHARED) {
        ...
        shared += lock_value;
        *lock_object = ((uint64_t)exclusive) << 32 | shared;
        client->GrantLock(elem->seq_no, elem->target_node_id, elem->owner_user_id,
            obj_index, elem->type);
        elem = queue->Front();
...
int LockManager::HandleWorkCompletion(struct ibv_wc* work_completion) {
  ...
  if (work_completion->status != IBV_WC_SUCCESS) {
    cerr << "(LockManager) Work completion status is not IBV_WC_SUCCESS." << endl;
    return -1;
  }
