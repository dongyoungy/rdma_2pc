...
// constructor
LocalLockManager::LocalLockManager(int node_id) {
...
int LocalLockManager::CheckLock(int seq_no, int owner_thread_id, int target_node_id,
  ...
  if (lock_status_[target_node_id][target_obj_index] != LOCK_STATUS_IDLE) {
    return LOCAL_LOCK_FAIL;
  }

  if (shared_counter_[target_node_id][target_obj_index] > 0 ||
      exclusive_counter_[target_node_id][target_obj_index] > 0) {
    ...
    if (wait_queue_[target_node_id][target_obj_index].size() > 0) {
      LocalLockWaitElement elem = wait_queue_[target_node_id][target_obj_index].front();
...
int LocalLockManager::Lock(int target_node_id, int target_obj_index, int lock_type) {
  if (lock_type == EXCLUSIVE) {
    exclusive_counter_[target_node_id][target_obj_index]++;
  } else if (lock_type == SHARED) {
    shared_counter_[target_node_id][target_obj_index]++;
  } else {
    return FUNC_FAIL;
  }
  return FUNC_SUCCESS;
...
int LocalLockManager::GetCount(int target_node_id, int target_obj_index, int lock_type) {
  if (lock_type == EXCLUSIVE) {
    return exclusive_counter_[target_node_id][target_obj_index];
  } else if (lock_type == SHARED) {
    return shared_counter_[target_node_id][target_obj_index];
  }
  return FUNC_FAIL;
}

int LocalLockManager::GetStatus(int target_node_id, int target_obj_index) {
  return lock_status_[target_node_id][target_obj_index];
}

void LocalLockManager::SetStatus(int target_node_id, int target_obj_index, int status) {
  lock_status_[target_node_id][target_obj_index] = status;
}

queue<LocalLockWaitElement>& LocalLockManager::GetQueue(int target_node_id,
    int target_obj_index) {
  return wait_queue_[target_node_id][target_obj_index];
}
