...
class NotifyLockClient : public LockClient {
 public:
  ...
  ~NotifyLockClient();
  virtual int RequestLock(int seq_no, uint32_t user_id, LockType lock_type,
                          int obj_index);
  virtual int RequestUnlock(int seq_no, uint32_t user_id, LockType lock_type,
                            int obj_index);

  int TryLock(int seq_no, int user_id, LockType lock_type, int obj_index);
 ...
 protected:
  ...
  int ReadForUnlock(Context* context, int seq_no, uint32_t user_id,
                    LockType lock_type, int obj_index);
  int UnlockRemotely(Context* context, int seq_no, uint32_t user_id,
                     LockType lock_type, int obj_index, bool is_undo = false);
  int UnlockRemotelyCS(Context* context, int seq_no, uint32_t user_id,
                       LockType lock_type, int obj_index, uint64_t prev_value,
                       uint64_t new_value);
  int LockRemotelyCS(Context* context, int seq_no, uint32_t user_id,
                     LockType lock_type, int obj_index, uint64_t prev_value);
