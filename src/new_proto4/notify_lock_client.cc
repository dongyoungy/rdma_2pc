...
int NotifyLockClient::RequestLock(int seq_no, uint32_t user_id,
                                  LockType lock_type, int obj_index) {
  return this->LockRemotelyCS(context_, seq_no, user_id, lock_type, obj_index,
                              0);  // assume zero
}

int NotifyLockClient::RequestUnlock(int seq_no, uint32_t user_id,
                                    LockType lock_type, int obj_index) {
  // return this->UnlockRemotely(context_, seq_no, user_id, lock_type,
  // obj_index);
...
int NotifyLockClient::UnlockRemotely(Context* context, int seq_no,
                                     uint32_t user_id, LockType lock_type,
  ...
  if ((ret = ibv_exp_post_send(context->queue_pair, &send_work_request,
                               &bad_work_request))) {
    cerr << "NotifyLockClient::UnlockRemotely(): ibv_exp_post_send() failed: "
         << strerror(ret) << endl;
    return -1;
  }
  ++num_rdma_atomic_;
...
int NotifyLockClient::LockRemotelyCS(Context* context, int seq_no,
                                     uint32_t user_id, LockType lock_type,
  ...
  if (lock_type == SHARED) {
    ...
    shared = local_owner_bitvector_id_;
    uint64_t new_value = ((uint64_t)exclusive) << 32 | shared;
    send_work_request.wr.atomic.swap = (prev_value | new_value);
  } else if (lock_type == EXCLUSIVE) {
  ...
  if ((ret = ibv_exp_post_send(context->queue_pair, &send_work_request,
                               &bad_work_request))) {
    cerr << "NotifyLockClient::LockRemotelyCS(): ibv_exp_post_send() failed: "
         << strerror(ret) << endl;
    return -1;
  }
  ++num_rdma_atomic_;
...
int NotifyLockClient::UnlockRemotelyCS(Context* context, int seq_no,
                                       uint32_t user_id, LockType lock_type,
                                       int obj_index, uint64_t prev_value,
                                       uint64_t new_value) {
  Poco::Mutex::ScopedLock lock(lock_mutex_);
...
int NotifyLockClient::TryLock(int seq_no, int user_id, LockType lock_type,
                              int obj_index) {
  // pthread_mutex_lock(&PRINT_MUTEX);
  // cout << "TryLock(): " << seq_no << "," << user_id << "," << obj_index <<
  // "," << lock_type << endl;  pthread_mutex_unlock(&PRINT_MUTEX);
...
int NotifyLockClient::NotifyWaitingNodes(LockRequest* request, uint64_t value) {
  ...
  if (request->lock_type == SHARED) {
    ...
    for (int i = 0; i < sz; ++i) {
      ...
      local_manager_->GrantLock(request->seq_no, remote_lm_id_, nodes[i],
  ...
  if (work_completion->opcode == IBV_WC_RECV) {
    ...
    if (request->task == READ) {
      ...
      if (lock_type == SHARED) {
        if ((shared & user_id) != 0) {
          ...
          this->UnlockRemotelyCS(context_, request->seq_no, request->user_id,
                                 request->lock_type, request->obj_index, value,
                                 new_value);
        } else {
    ...
    if (request->task == LOCK) {
      if (value == request->prev_value) {
        ...
        this->LockRemotelyCS(context_, request->seq_no, request->user_id,
                             request->lock_type, request->obj_index, value);
      }
    } else if (request->task == UNLOCK) {
      if (value == request->prev_value) {
