...
resource_str="span[ptile=1] affinity[core(10,same=numa):cpubind=numa:membind=localprefer] rusage[mem=6144]"
job_name="dist_lock_tpcc"
...
for num_node in 2
do
  for num_warehouse_per_node in 4
  do
    for num_user in 16 # per node
    do 
        ...
        do
          ...
          do
            ...
            bsub -n $num_node -a openmpi -W $wall_time \
              ...
              -eo /gpfs/gpfs0/groups/mozafari/dyoon/work/lsf_log/sosp2017/$lock_method_str-$workload_str-$s_e_str-$e_s_str-$e_e_str-fr$num_retry-pr$num_retry-st$sleep_time-tt$think_time-bmin$min_backoff-bmax$max_backoff-nn$num_node-nw$num_warehouse_per_node-u$num_user-tx$num_tx-simultx$simulate_tx_delay-$simulate_tx_delay_min-$simulate_tx_delay_max.e \
              -oo /gpfs/gpfs0/groups/mozafari/dyoon/work/lsf_log/sosp2017/$lock_method_str-$workload_str-$s_e_str-$e_s_str-$e_e_str-fr$num_retry-pr$num_retry-st$sleep_time-tt$think_time-bmin$min_backoff-bmax$max_backoff-nn$num_node-nw$num_warehouse_per_node-u$num_user-tx$num_tx-simultx$simulate_tx_delay-$simulate_tx_delay_min-$simulate_tx_delay_max.o -q normal \
              "sh tpcc_test.lsf $workload_type $num_tx $num_warehouse_per_node $num_user $lock_method $s_e_rule $e_s_rule $e_e_rule $num_retry $num_retry $sleep_time $think_time $simulate_tx_delay $simulate_tx_delay_min $simulate_tx_delay_max $min_backoff $max_backoff $rand_seed"
            ...
            #let lock_method=4 # direct queue
            #let sleep_time=0
            #if [ $lock_method -eq 0 ]
            #then
              #lock_method_str="proxy-retry"
...
echo "conflux job done" | mail -s "conflux job finished" dyoon@umich.edu
